{{--
    Delivery checklist row. Dispatches between two row types via $row['_type']:
...
@if (($row['_type'] ?? 'item') === 'group')
    <tr>
        <td colspan="5" style="background: #e8eef9; font-weight: bold; font-size: 10px; padding: 5px 8px; border: 1px solid #cbd5e1;">
            ...
            @endif
            @if (! empty($row['owner_phone']) && $row['owner_phone'] !== 'N/A')
                <span style="font-weight: normal; color: #475569; font-size: 9px;">&nbsp;&middot;&nbsp; {{ $row['owner_phone'] }}</span>
            @endif
...
@else
    <tr class="{{ $alt ? 'alt' : '' }}">
        ...
        <td class="m">{{ $row['expected_delivery'] }}</td>
        <td>
