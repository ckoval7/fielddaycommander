...
beforeEach(function () {
    ...
    $this->bt = BonusType::where('event_type_id', $this->event->event_type_id)
        ...
        ->where('code', 'agency_visit')
        ->first();
});

it('reports code and trigger type', function () {
    $s = new AgencyVisitStrategy;
    expect($s->code())->toBe('agency_visit')
        ->and($s->triggerType())->toBe('derived')
        ->and($s->subscribesTo())->toBe([GuestbookEntryChanged::class]);
});

it('writes the row when a verified agency entry exists', function () {
    ...
    (new AgencyVisitStrategy)->reconcile($this->config);
...
it('is idempotent', function () {
    ...
    $strategy = new AgencyVisitStrategy;
