...
test('writeOrDelete preserves claimed_by_user_id and verified_at on update', function () {
    ...
    $originalVerifiedAt = $original->verified_at;

    $strategy = new class extends AbstractBonusStrategy
    {
        public function code(): string
        {
            return 'test_code';
        ...
        public function callWriteOrDelete($config, $bonusType, $quantity, $points): void
        {
            $this->writeOrDelete($config, $bonusType, $quantity, $points);
        }
    };

    $strategy->callWriteOrDelete($config, $bonusType, 2, 10);

    $fresh = $original->fresh();

    expect($fresh->quantity)->toBe(2)
        ->and((int) $fresh->calculated_points)->toBe(10)
        ->and($fresh->is_verified)->toBeTrue()
        ->and($fresh->claimed_by_user_id)->toBe($user->id)
        ->and($fresh->verified_by_user_id)->toBe($user->id)
        ->and($fresh->verified_at->equalTo($originalVerifiedAt))->toBeTrue();
});
