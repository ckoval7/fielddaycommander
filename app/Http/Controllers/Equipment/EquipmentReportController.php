...
class EquipmentReportController extends Controller
{
    ...
    /**
     * Human-readable report titles, keyed by report type slug.
     * Mirrors the `report_title` set in the matching blade view's @extends.
     *
     * @var array<string, string>
     */
    private const REPORT_TITLES = [
        'delivery-checklist' => 'Equipment Delivery Checklist',
        'station-inventory' => 'Station Equipment Inventory',
        'owner-contacts' => 'Equipment Owner Contact List',
        'return-checklist' => 'Equipment Return Checklist',
        'incident-report' => 'Equipment Incident Report',
    ];
    ...
    protected function exportPdf(string $view, array $data, Event $event, string $reportType): Response
    {
        ...
        $generatedAt = now();
        $reportTitle = self::REPORT_TITLES[$reportType] ?? Str::headline($reportType);

        $pdf = Pdf::loadView($view, $data + ['generated_at' => $generatedAt])
            ->setPaper('letter', 'portrait');

        // Draw the per-page footer via canvas so we get DomPDF's {PAGE_NUM} /
        // {PAGE_COUNT} placeholders (CSS counter(pages) is not implemented in
        // dompdf 3.x — it always returns 0).
        $pdf->render();
        ...
        $fontSize = 9;
        $textColor = [0.28, 0.34, 0.41]; // #475569
        $ruleColor = [0.118, 0.243, 0.659]; // #1e3ea8
        $pageWidth = $canvas->get_width();
        $marginX = 48.0;
        $baselineY = $canvas->get_height() - 36;
        $ruleY = $baselineY - 8;

        $left = sprintf('Generated by Field Day Commander — %s UTC', $generatedAt->format(self::DATETIME_FORMAT));
        // Event name already appears in the per-page header bar, so don't
        // repeat it here — keeps the footer short enough to fit on one line.
        $right = sprintf('%s  ·  Page {PAGE_NUM} of {PAGE_COUNT}', $reportTitle);
        // {PAGE_NUM} / {PAGE_COUNT} get substituted at finalization, but the
        // x-position passed to page_text is fixed up-front. Measure the width
        // against the widest possible substituted form (page-num digits ≤
        // total page count digits) so right-alignment lands correctly.
        $pageCount = max(1, $canvas->get_page_count());
        $rightForMeasurement = str_replace(['{PAGE_NUM}', '{PAGE_COUNT}'], (string) $pageCount, $right);
        $rightWidth = $fontMetrics->getTextWidth($rightForMeasurement, $font, $fontSize);

        $canvas->page_line($marginX, $ruleY, $pageWidth - $marginX, $ruleY, $ruleColor, 1.5);
        $canvas->page_text($marginX, $baselineY, $left, $font, $fontSize, $textColor);
        $canvas->page_text($pageWidth - $marginX - $rightWidth, $baselineY, $right, $font, $fontSize, $textColor);
